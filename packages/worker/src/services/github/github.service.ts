...
export class GithubService {
  ...
  static async getMembersListProcess(createdAt?: string) {
    ...
    const load: Search = await graphqlWithAuth(`
      query {
        search(query: "repo:khulnasoft/teleflow is:pr is:merged sort:created-asc ${createdAt ? `created:>${createdAt}` : ''}", type: ISSUE, first: 100) {
          issueCount
          edges {
            node {
              ... on PullRequest {
  ...
  static async loadPullRequests(): Promise<PullsList[]> {
    ...
    for (const name of names) {
      try {
        const info = await GithubService.loadPulls(name);
        pulls.push(...info);
      } catch (err) {
  ...
  static async GithubLoadIssues(): Promise<any> {
    const { data } = await github.get(`/repos/khulnasoft/teleflow/issues?per_page=100`);
    return data;
  }
  ...
  static async loadRepositories(): Promise<string[]> {
    const { data } = await github.get('/orgs/khulnasoft/repos?type=public');
    return data.map((d: any) => d.name);
  }

  static async loadPulls(name: string, page = 1): Promise<PullsList[]> {
    const path = `/repos/khulnasoft/${name}/pulls?state=closed&per_page=30&page=${page}`;
    console.log(`- Getting: ${path}`);
    const { data } = await github.get(path);
    return [
      ...data,
      ...(data.length ? await GithubService.loadPulls(name, page + 1) : []),
    ];
  }
}
